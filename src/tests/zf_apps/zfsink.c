...
static void poll_muxer(struct resources* res,
...
static void ev_loop_muxer(struct resources* res, struct zf_muxer_set* muxer)
{
  while( 1 )
    poll_muxer(res, muxer, -1);
}
...
int main(int argc, char* argv[])
{
  ...
  if( cfg_waitable_fd )
    ev_loop_waitable_fd(res, stack, muxer);
  else if( cfg_muxer )
    ev_loop_muxer(res, muxer);
  else
    ev_loop_reactor(res, stack, ur);
